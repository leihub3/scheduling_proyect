...
if($_GET["q"] != ""){
$sWhere = " where anio like '%$anio%' && periodo like '%$semestre%' && profesor like '%$q%'";
}else{
$sWhere = " where anio='$anio' && periodo='$semestre'";
}

$sql = "$sTable $sWhere order by curso asc";
//echo $sql;
$res = $con->query($sql);
if($res->num_rows > 0){	
...
<td colspan="2"><label>Periodo Actual: </label><input type="text" class="form-control" name="periodo_actual" readonly  value="<?php echo $_GET["p"]; ?>"> </td>
